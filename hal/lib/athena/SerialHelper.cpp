...
std::vector<std::string> SerialHelper::GetVISASerialPortList(int32_t* status) {
  ...
  retVec.push_back(std::string(OnboardResourceVISA));
  retVec.push_back(std::string(MxpResourceVISA));
  ...
  for (size_t i = 0; i < retCnt; i++, viFindNext(viList, desc)) {
    ...
    *status = 0;
    ...
    if (!output.empty()) {
      ...
      m_unsortedHubPath.push_back(llvm::StringRef{pathSplitVec[hubIndex - 2]});
