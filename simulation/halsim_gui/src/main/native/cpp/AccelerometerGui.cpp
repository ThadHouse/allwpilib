...
static void DisplayAccelerometer() {
  if (!HALSIM_GetAccelerometerActive(0)) return;
  if (ImGui::TreeNodeEx("BuiltInAccel", ImGuiTreeNodeFlags_DefaultOpen)) {
    HAL_Value value;
...
void AccelerometerGui::Initialize() { SimDeviceGui::Add(DisplayAccelerometer); }

void AccelerometerGui::Execute() {}
